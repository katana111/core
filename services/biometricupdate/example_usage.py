"""
Example usage of BiometricUpdate enrichment service
Demonstrates how to scrape and analyze company news from biometricupdate.com
"""
...
async def example_1_enrich_all_competitors():
    """Example 1: Enrich all competitors from database with BiometricUpdate articles"""
    
    print("🚀 Example 1: Enriching all competitors with BiometricUpdate articles")
    print("=" * 80)
    
    # Run enrichment for all competitors in database with AI analysis
    results = await enrich_competitors_with_biometricupdate(analyze_news=True)
...
async def example_3_single_company_scraping():
    ...
    # Search for articles
    print(f"🔍 Searching for '{company_name}' articles on BiometricUpdate...")
...
async def example_4_without_ai_analysis():
    ...
    db_ops = BiometricUpdateDataOperations(analyze_news=False)
...
async def main():
    ...
    print("🎯 BIOMETRICUPDATE SERVICE EXAMPLES")
    print("=" * 80)
    print("This will demonstrate different ways to use the BiometricUpdate service")
    print()
